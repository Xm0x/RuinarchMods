...
namespace RuinarchPlus.Phase5
{
	...
	public static class Famine
	{
		...
		// ---- persistence -------------------------------------------------------------------
		// One settlement persistent ID per village in famine.
		...
		private static string Save()
		{
			...
			foreach (KeyValuePair<NPCSettlement, State> kv in States)
			{
				...
				{
					file.famines.Add(kv.Key.persistentID);
				}
			}
		...
		private static void Load(string json)
		{
			...
			foreach (string id in JsonUtility.FromJson<FamineSaveData>(json)?.famines ?? new List<string>())
			{
				if (LandmarkManager.Instance.GetSettlementByPersistentID(id) is NPCSettlement s)
				{
					States[s] = new State { Active = true };
				}
			}
